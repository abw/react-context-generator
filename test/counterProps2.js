...
import App      from './lib/Counter2/App.js'
...
it('can render and update a counter', () => {
  ...
  act(() => {
    ReactDOM.createRoot(container).render(<App initialCount={200}/>);
  });
  const inc   = container.querySelector('#inc');
  const dec   = container.querySelector('#dec');
  const count = container.querySelector('#count');
  expect(count.textContent).toBe('The current count is 200');

  // increment the counter
  act(() => {
    inc.dispatchEvent(new MouseEvent('click', {bubbles: true}));
  });
  expect(count.textContent).toBe('The current count is 201');
